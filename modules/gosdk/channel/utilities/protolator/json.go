...
func (pf *plainField) PopulateFrom(source interface{}) error {
	if !reflect.TypeOf(source).AssignableTo(pf.fType) {
		common.Logger.Error(fmt.Sprintf("expected field %s for message %T to be assignable from %v but was not.  Is %T", pf.name, pf.msg, pf.fType, source))
...
func protoFieldsForDecode(msg proto.Message, uMsg proto.Message) ([]protoField, error) {
...
func recursivelyCreateTreeFromMessage(msg proto.Message) (tree map[string]interface{}, err error) {
	...
	fields, err := protoFieldsForDecode(msg, uMsg)
	if err != nil {
		return nil, err
	}
...
// DeepUnmarshalMap is copied from DeepUnmarshalJSON to takes config_group decodes it into msg
func DeepUnmarshalMap(r map[string]interface{}, msg proto.Message) error {
	return recursivelyPopulateMessageFromTree(r, msg)
}
