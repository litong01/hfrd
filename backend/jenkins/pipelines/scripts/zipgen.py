...
kubeOutputPath = '/fabric/keyfiles/'
topOutputDir = ""
outputDirFlag = False
allConnProfileMap = {}
allOrganizations = {}
...
def processOrdererSection(orderers, newSCFileObject):
    ...
    for key in orderers:
        ...
        kubeOrdererTLSCAPath = kubeOutputPath + 'orderers/'+key+'/tls/'
        ...
        fo.close()
        newSCFileObject['orderers'][key]['tlsCACerts']['path'] = kubeOrdererTLSCAPath+'ca.pem'
...
def processPeersSection(peers, newSCFileObject):
    ...
    for key in peers:
        ...
        kubePeerTLSCAPath = kubeOutputPath + tempOrgName+'/Peers/'+key+'/tls/'
        ...
        fo.close()
        newSCFileObject['peers'][key]['tlsCACerts']['path'] = kubePeerTLSCAPath+'ca.pem'
...
def processCAsSections(cas, newSCFileObject):
    ...
    for key in cas:
        ...
        kubeCaTLSCAPath = kubeOutputPath + 'certificateAuthorities/'+key+'/tls/'
        ...
        fo.close()
        newSCFileObject['certificateAuthorities'][key]['tlsCACerts']['path'] = kubeCaTLSCAPath+'ca.pem'
...
def enrollUserForOrg(cas, newSCFileObject, keyofOrg):
    ...
    tempMspPath = outputDir + keyofOrg+'/users/'+'Admin'+'@'+keyofOrg+'/msp'
    mspPath = tempMspPath
    cmdRet = 0
    # retry when error
    for i in range(0,20):
        try:
            os.system('rm -rf ' + mspPath)  # clean up msp directory
            ...
            if (cmdRet != 0):
                ...
                raise Exception('enroll user failed,return code=' + str(cmdRet))
        except Exception:
            print('enroll ' + caenrollUrl + ' for ' + keyofOrg + ' failed, will retry:' + str(i))
            time.sleep(5)
            continue
...
for keyofOrg in networkJson:
    ...
    try:
        ...
        print(
            'getting original connection profile for organization['+keyofOrg + '] from: '+connProfileUrl)
        tempHttpResp = requests.get(
            connProfileUrl, auth=authx, headers=heliosGetHeaders,verify=False)
        if tempHttpResp.status_code != requests.codes.ok:
            print('Error getting connection profile:' + str(tempHttpResp))
            sys.exit(2)
        ...
        newSCFileObject['client']['cryptoconfig']['path'] = '/fabric/keyfiles'
        ...
        allConnProfileMap[keyofOrg]['jsonFilePath'] = os.path.join(
            outputDir+orgItem_key+'/', 'connection.json')
        allConnProfileMap[keyofOrg]['yamlFilePath'] = os.path.join(
            outputDir+orgItem_key+'/', 'connection.yml')
...
os.system('cp ' + outputDir+'network.json  '+ topOutputDir)
os.system('chmod -R 755 '+outputDir)
shutil.make_archive(topOutputDir + 'ibpcerts',
                    'gztar', topOutputDir)
