...
if $SCFILE_CREATE ; then
	log "---------------------------------"
	log "---------------------------------"
	log "Generate PTE SCFile 			  "
	log "---------------------------------"
	log "---------------------------------"
	# Gather MSPIDs
	gatherMSPIDS $HOME/results/creds/
	networkId=$(jq -r .${MSP_IDs[0]}.network_id $HOME/results/creds/network.json)
	# Use python script to generate PTE SCFile.Results will be stored in ${HOME}/results/SCFiles/  with name 'config-net-${networkId}.json'
	python ${WORKDIR}/scripts/create_SCFile.py -m ${MSP_IDs[@]} -n $networkId
	if [ $? -ne 0 ]; then
		exit 2
	fi
...
if $WORKLOAD_DRIVE ; then
	log "---------------------------------"
	log "---------------------------------"
	log "Drive PTE workload: $workload	  "
	log "---------------------------------"
	log "---------------------------------"
	if [[ $name == 'ep' ]]; then
		sed -i "s/PEER_ORG/PeerOrg1/g" ${WORKDIR}/workloads/$workload/*
	else
		sed -i "s/PEER_ORG/org1/g" ${WORKDIR}/workloads/$workload/*
	fi
	if [[ $runMode == 'local' ]]; then
		...
		./cloud-test/workload_driver_cloud.sh -w $workload -n $HOME/results/creds/network.json -c $WORKDIR/hfrd_test.cfg
		if [ $? -ne 0 ]; then
			exit 2
		fi
	fi
