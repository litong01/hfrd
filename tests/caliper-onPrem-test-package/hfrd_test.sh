...
# Build caliper-fab image with same user name/id and group id
docker build -t caliper-fab --build-arg uid=$(id -u) --build-arg gid=$(id -g) .
...
if [[ $env == 'cm' ]];then
  ...
  docker run --rm -v $(pwd)/conf/hosts:/etc/hosts -v $RESULTS_DIR:${HOME_COTNAINER}/results/ caliper-fab /bin/bash -c "$startCmd"
else
	docker run --rm -v $RESULTS_DIR:${HOME_COTNAINER}/results/ caliper-fab /bin/bash -c "$startCmd"
fi